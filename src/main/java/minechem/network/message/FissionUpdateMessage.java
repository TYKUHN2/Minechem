...
public class FissionUpdateMessage implements IMessage, IMessageHandler<FissionUpdateMessage, IMessage> {
	...
	public FissionUpdateMessage(TileFissionCore tile, boolean doLink) {
		pos = tile.getPos();
	...
	public IMessage onMessage(FissionUpdateMessage message, MessageContext ctx) {
		...
		if (tileEntity != null) {
			...
			if (energyStorage != null) {
				...
				//	((EnergySettableMultiBlock) energyStorage).setEnergy(message.energyStored);
				//}
				/*else*/ if (energyStorage instanceof EnergySettable) {
