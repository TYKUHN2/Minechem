...
public class CharacterSprite extends TextureAtlasSprite {
	...
	private CharacterSprite(char character) {
		super(ModGlobals.ID + ":charsprite_" + String.valueOf(character) + "" + (Character.isUpperCase(character) ? "_upper" : ""));
	...
	public boolean load(IResourceManager manager, ResourceLocation location, Function<ResourceLocation, TextureAtlasSprite> textureGetter) {
		...
		int xPos = getCharPos(character).getLeft();
		int yPos = getCharPos(character).getRight();
		...
		*/
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				...
				int color = i >= 8 || j >= 8 ? 0x00000000 : charmap_image.getRGB(i + xPos, j + yPos);
	...
	public static ResourceLocation getFontTexture() {
		if (FONT_TEXTURE == null) {
			FONT_TEXTURE = ReflectionHelper.getPrivateValue(FontRenderer.class, Minecraft.getMinecraft().fontRenderer, "locationFontTexture", "field_111273_g");
		}
		return FONT_TEXTURE;
	}
